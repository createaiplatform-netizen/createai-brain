  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <button onClick={onBack} className="text-primary text-sm font-medium">‹ Back</button>
        <div className="flex-1" />
        <button onClick={handleCopy} className="text-[11px] bg-muted border border-border/50 rounded-xl px-3 py-1.5 hover:bg-muted/80 transition-colors">
          {copied ? "✓ Copied!" : "Copy"}
        </button>
        <button onClick={handleSave} className="text-[11px] bg-green-500 text-white rounded-xl px-3 py-1.5 hover:opacity-90 transition-opacity">
          {saved ? "✓ Saved!" : "Save"}
        </button>
      </div>
      <div>
        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">{item.type.toUpperCase()} · {item.createdAt.toLocaleTimeString()}</p>
        <h3 className="text-base font-bold text-foreground">{item.label}</h3>
      </div>
      <div className="bg-muted/30 border border-border/40 rounded-2xl p-4 max-h-[60vh] overflow-y-auto">
        <pre className="text-[12px] text-foreground whitespace-pre-wrap leading-relaxed font-mono">{item.content}</pre>
      </div>
      <p className="text-[10px] text-muted-foreground text-center">All content is generated by CreateAI Brain · Internal use · Fictional</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl p-4 text-white">
        <p className="text-[11px] font-bold opacity-70 uppercase mb-3">Generate Content Now</p>
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            <button key={a.label} onClick={() => setView("generate" as MarketingView)}
              className="bg-white/20 hover:bg-white/30 text-white text-[12px] font-semibold rounded-xl py-2 transition-colors">
              {a.label}
            </button>
          ))}
        </div>
...
export function MarketingApp() {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Top nav */}
      <div className="flex-none border-b border-border/30 bg-background/80 backdrop-blur-sm">
        <div className="flex overflow-x-auto px-2 py-1.5 gap-0.5 no-scrollbar">
          ...
          <div className="ml-auto flex items-center px-2">
            <span className="text-[10px] text-muted-foreground">{history.length} generated</span>
          </div>
        </div>
      </div>
