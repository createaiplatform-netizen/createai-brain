...
import { WARLORE_ENGINES, WARLORE_SERIES, runWarloreEngine, type WarloreEngineDefinition, type WarloreSeriesDefinition } from "@/engine/WarloreEngine";
...
const ACCENT = "#dc2626"; const ICON = "⚔️"; const LABEL = "WarloreStudio"; const API = "/api/warlore";
function fmt(ts: string) { return new Date(ts).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }); }

function EngineCard({ e, onSelect }: { e: WarloreEngineDefinition; onSelect: () => void }) {
  return (
...
function RunPanel({ engine, onBack }: { engine: WarloreEngineDefinition; onBack: () => void }) {
  ...
  const run = useCallback(async () => {
    ...
    await runWarloreEngine({ engineId: engine.id, engineName: engine.name, topic: topic.trim(),
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {WARLORE_SERIES.map((s: WarloreSeriesDefinition) => (
...
export function WarloreStudioApp() {
  const [view, setView] = useState<View>("grid"); const [activeEngine, setActiveEngine] = useState<WarloreEngineDefinition | null>(null);
  const [viewingSession, setViewingSession] = useState<SavedSession | null>(null); const [search, setSearch] = useState("");
  const filtered = search.trim() ? WARLORE_ENGINES.filter(e => e.name.toLowerCase().includes(search.toLowerCase())) : WARLORE_ENGINES;
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      <div style={{ padding: "20px 24px 0", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 12 }}>
          ...
          <div><h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: "#f1f5f9" }}>{LABEL}</h2><p style={{ margin: 0, fontSize: 12, color: "#64748b" }}>Military & Warfare Design · {WARLORE_ENGINES.length} engines · {WARLORE_SERIES.length} series</p></div>
        </div>
