...
import { URBANWORLD_ENGINES, URBANWORLD_SERIES, runUrbanWorldEngine, type UrbanWorldEngineDefinition, type UrbanWorldSeriesDefinition } from "@/engine/UrbanWorldEngine";
...
const ACCENT = "#6366f1"; const ICON = "🏙️"; const LABEL = "UrbanWorldEngine"; const API = "/api/urbanworld";
function fmt(ts: string) { return new Date(ts).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }); }

function EngineCard({ e, onSelect }: { e: UrbanWorldEngineDefinition; onSelect: () => void }) {
  return (
...
function RunPanel({ engine, onBack }: { engine: UrbanWorldEngineDefinition; onBack: () => void }) {
  ...
  const run = useCallback(async () => {
    ...
    await runUrbanWorldEngine({ engineId: engine.id, engineName: engine.name, topic: topic.trim(),
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {URBANWORLD_SERIES.map((s: UrbanWorldSeriesDefinition) => (
...
export function UrbanWorldEngineApp() {
  const [view, setView] = useState<View>("grid"); const [activeEngine, setActiveEngine] = useState<UrbanWorldEngineDefinition | null>(null);
  const [viewingSession, setViewingSession] = useState<SavedSession | null>(null); const [search, setSearch] = useState("");
  const filtered = search.trim() ? URBANWORLD_ENGINES.filter(e => e.name.toLowerCase().includes(search.toLowerCase())) : URBANWORLD_ENGINES;
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      <div style={{ padding: "20px 24px 0", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 12 }}>
          <div style={{ width: 44, height: 44, borderRadius: 12, background: `linear-gradient(135deg,${ACCENT},#374151)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22 }}>{ICON}</div>
          <div><h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: "#f1f5f9" }}>{LABEL}</h2><p style={{ margin: 0, fontSize: 12, color: "#64748b" }}>City & Urban World Design · {URBANWORLD_ENGINES.length} engines · {URBANWORLD_SERIES.length} series</p></div>
        </div>
