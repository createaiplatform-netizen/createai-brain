...
function EngineCard({ engine, onSelect }: { engine: MythweaveEngineDefinition; onSelect: () => void }) {
  return (
...
function RunPanel({ engine, onBack }: { engine: MythweaveEngineDefinition; onBack: () => void }) {
  ...
  const save = useCallback(async () => {
    ...
    try { await fetch("/api/mythweave", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ engineId: engine.id, engineName: engine.name, topic: topic.trim(), output }) }); setSaved(true); } catch { /* silent */ }
...
function SeriesPanel({ onBack }: { onBack: () => void }) {
  const [activeSeries, setActiveSeries] = useState<MythweaveSeriesDefinition | null>(null);
  const [topic, setTopic] = useState(""); const [output, setOutput] = useState(""); const [running, setRunning] = useState(false);
  const outputRef = useRef<HTMLDivElement>(null);
  ...
  if (activeSeries) return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", gap: 16 }}>
      ...
      <textarea value={topic} onChange={e => setTopic(e.target.value)} placeholder="Enter your myth topic for the full series run…" rows={3}
        style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 10, padding: "10px 14px", color: "#e2e8f0", fontSize: 13, fontFamily: "inherit", resize: "none" }} />
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: "#94a3b8", cursor: "pointer", fontSize: 20 }}>←</button>
        <div><div style={{ fontWeight: 700, fontSize: 16, color: "#f1f5f9" }}>🕸️ Mythweave Series</div><div style={{ fontSize: 12, color: "#64748b" }}>Run multiple engines in sequence on one mythic arc</div></div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 12 }}>
        {MYTHWEAVE_SERIES.map(s => (
...
function SessionsPanel({ onBack, onView }: { onBack: () => void; onView: (s: SavedSession) => void }) {
  ...
  useEffect(() => { fetch("/api/mythweave", { credentials: "include" }).then(r => r.ok ? r.json() : { sessions: [] }).then((d: { sessions?: SavedSession[] }) => setSessions(d.sessions ?? [])).catch(() => {}).finally(() => setLoading(false)); }, []);
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: "#94a3b8", cursor: "pointer", fontSize: 20 }}>←</button>
        <div style={{ fontWeight: 700, fontSize: 16, color: "#f1f5f9" }}>Saved Myths ({sessions.length})</div>
      </div>
      {loading ? <div style={{ color: "#64748b", fontSize: 13 }}>Loading…</div>
        : sessions.length === 0 ? <div style={{ textAlign: "center", padding: "40px 0", color: "#64748b", fontSize: 13 }}>No saved sessions yet. Run an engine and save your myth!</div>
        : sessions.map(s => (
...
export function MythweaveStudioApp() {
  const [view, setView] = useState<View>("grid"); const [activeEngine, setActiveEngine] = useState<MythweaveEngineDefinition | null>(null); const [viewingSession, setViewingSession] = useState<SavedSession | null>(null); const [search, setSearch] = useState("");
  const filtered = search.trim() ? MYTHWEAVE_ENGINES.filter(e => e.name.toLowerCase().includes(search.toLowerCase()) || e.description.toLowerCase().includes(search.toLowerCase())) : MYTHWEAVE_ENGINES;
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      <div style={{ padding: "20px 24px 0", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 12 }}>
          <div style={{ width: 44, height: 44, borderRadius: 12, background: "linear-gradient(135deg,#7c3aed,#374151)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22 }}>🕸️</div>
          <div><h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: "#f1f5f9" }}>MythweaveStudio</h2><p style={{ margin: 0, fontSize: 12, color: "#64748b" }}>Myth, Archetype & Symbol Designer · {MYTHWEAVE_ENGINES.length} engines · {MYTHWEAVE_SERIES.length} series</p></div>
        </div>
