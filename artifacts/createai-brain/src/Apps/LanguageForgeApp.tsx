...
import { LANGUAGEFORGE_ENGINES, LANGUAGEFORGE_SERIES, runLanguageForgeEngine, type LanguageForgeEngineDefinition, type LanguageForgeSeriesDefinition } from "@/engine/LanguageForgeEngine";
...
const ACCENT = "#6366f1"; const ICON = "🔤"; const LABEL = "LanguageForge"; const API = "/api/languageforge";
function fmt(ts: string) { return new Date(ts).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }); }

function EngineCard({ e, onSelect }: { e: LanguageForgeEngineDefinition; onSelect: () => void }) {
  return (
...
function RunPanel({ engine, onBack }: { engine: LanguageForgeEngineDefinition; onBack: () => void }) {
  ...
  const run = useCallback(async () => {
    ...
    await runLanguageForgeEngine({ engineId: engine.id, engineName: engine.name, topic: topic.trim(),
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", gap: 16 }}>
      ...
      <button onClick={run} disabled={running || !topic.trim()} style={{ background: running ? "rgba(99,102,241,0.3)" : engine.color, border: "none", borderRadius: 10, padding: "11px 20px", color: "#fff", fontSize: 14, fontWeight: 700, cursor: running ? "not-allowed" : "pointer" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {LANGUAGEFORGE_SERIES.map((s: LanguageForgeSeriesDefinition) => (
...
export function LanguageForgeApp() {
  const [view, setView] = useState<View>("grid"); const [activeEngine, setActiveEngine] = useState<LanguageForgeEngineDefinition | null>(null);
  const [viewingSession, setViewingSession] = useState<SavedSession | null>(null); const [search, setSearch] = useState("");
  const filtered = search.trim() ? LANGUAGEFORGE_ENGINES.filter(e => e.name.toLowerCase().includes(search.toLowerCase()) || e.description.toLowerCase().includes(search.toLowerCase())) : LANGUAGEFORGE_ENGINES;
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      <div style={{ padding: "20px 24px 0", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 12 }}>
          <div style={{ width: 44, height: 44, borderRadius: 12, background: `linear-gradient(135deg,${ACCENT},#0891b2)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22 }}>{ICON}</div>
          <div><h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: "#f1f5f9" }}>{LABEL}</h2><p style={{ margin: 0, fontSize: 12, color: "#64748b" }}>Constructed Language Studio · {LANGUAGEFORGE_ENGINES.length} engines · {LANGUAGEFORGE_SERIES.length} series</p></div>
        </div>
