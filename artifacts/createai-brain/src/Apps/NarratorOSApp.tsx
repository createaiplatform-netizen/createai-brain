...
function ActiveSeriesRunPanel({ series, accentColor, onBack }: { series: NarratorSeriesDefinition; accentColor: string; onBack: () => void }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", gap: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div><div style={{ fontWeight: 700, fontSize: 15, color: "#f1f5f9" }}>{series.name}</div><div style={{ fontSize: 11, color: "#94a3b8" }}>{series.engines.length} engines in sequence · ~{series.estimatedMinutes}min</div></div>
      </div>
      ...
      <textarea value={topic} onChange={e => setTopic(e.target.value)} placeholder="Enter your screenplay topic for the full series run…" rows={3}
        style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 10, padding: "10px 14px", color: "#e2e8f0", fontSize: 13, fontFamily: "inherit", resize: "none" }} />
