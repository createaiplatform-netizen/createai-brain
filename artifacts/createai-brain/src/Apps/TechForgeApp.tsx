...
import { TECHFORGE_ENGINES, TECHFORGE_SERIES, runTechForgeEngine, type TechForgeEngineDefinition, type TechForgeSeriesDefinition } from "@/engine/TechForgeEngine";
...
const ACCENT = "#0891b2"; const ICON = "🛸"; const LABEL = "TechForge"; const API = "/api/techforge";
function fmt(ts: string) { return new Date(ts).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }); }

function EngineCard({ e, onSelect }: { e: TechForgeEngineDefinition; onSelect: () => void }) {
  return (
...
function RunPanel({ engine, onBack }: { engine: TechForgeEngineDefinition; onBack: () => void }) {
  ...
  const run = useCallback(async () => {
    ...
    await runTechForgeEngine({ engineId: engine.id, engineName: engine.name, topic: topic.trim(),
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {TECHFORGE_SERIES.map((s: TechForgeSeriesDefinition) => (
          <button key={s.id} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14, padding: "16px 18px", textAlign: "left", cursor: "pointer" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}><span style={{ fontSize: 22 }}>{s.icon}</span><div><div style={{ fontWeight: 700, fontSize: 14, color: "#f1f5f9" }}>{s.name}</div><div style={{ fontSize: 11, color: "#94a3b8" }}>{s.engines.length} engines · ~{s.estimatedMinutes}min</div></div></div>
...
export function TechForgeApp() {
  const [view, setView] = useState<View>("grid"); const [activeEngine, setActiveEngine] = useState<TechForgeEngineDefinition | null>(null);
  const [viewingSession, setViewingSession] = useState<SavedSession | null>(null); const [search, setSearch] = useState("");
  const filtered = search.trim() ? TECHFORGE_ENGINES.filter(e => e.name.toLowerCase().includes(search.toLowerCase())) : TECHFORGE_ENGINES;
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      <div style={{ padding: "20px 24px 0", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 12 }}>
          ...
          <div><h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: "#f1f5f9" }}>{LABEL}</h2><p style={{ margin: 0, fontSize: 12, color: "#64748b" }}>Fictional Technology Design · {TECHFORGE_ENGINES.length} engines · {TECHFORGE_SERIES.length} series</p></div>
        </div>
