...
import { CHARACTERFORGE_ENGINES, CHARACTERFORGE_SERIES, runCharacterForgeEngine, type CharacterForgeEngineDefinition, type CharacterForgeSeriesDefinition } from "@/engine/CharacterForgeEngine";
...
const ACCENT = "#be185d"; const ICON = "🧠"; const LABEL = "CharacterForge"; const API = "/api/characterforge";
function fmt(ts: string) { return new Date(ts).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }); }

function EngineCard({ e, onSelect }: { e: CharacterForgeEngineDefinition; onSelect: () => void }) {
  return (
...
function RunPanel({ engine, onBack }: { engine: CharacterForgeEngineDefinition; onBack: () => void }) {
  ...
  const run = useCallback(async () => {
    ...
    await runCharacterForgeEngine({ engineId: engine.id, engineName: engine.name, topic: topic.trim(),
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", gap: 16 }}>
      ...
      <textarea value={topic} onChange={e => setTopic(e.target.value)} placeholder={engine.placeholder} rows={3}
      ...
      <button onClick={run} disabled={running || !topic.trim()} style={{ background: running ? ACCENT + "50" : engine.color, border: "none", borderRadius: 10, padding: "11px 20px", color: "#fff", fontSize: 14, fontWeight: 700, cursor: running ? "not-allowed" : "pointer" }}>
        {running ? "⟳ Running…" : `${engine.icon} Run ${engine.name}`}
      </button>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {CHARACTERFORGE_SERIES.map((s: CharacterForgeSeriesDefinition) => (
...
export function CharacterForgeApp() {
  const [view, setView] = useState<View>("grid"); const [activeEngine, setActiveEngine] = useState<CharacterForgeEngineDefinition | null>(null);
  const [viewingSession, setViewingSession] = useState<SavedSession | null>(null); const [search, setSearch] = useState("");
  const filtered = search.trim() ? CHARACTERFORGE_ENGINES.filter(e => e.name.toLowerCase().includes(search.toLowerCase())) : CHARACTERFORGE_ENGINES;
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      <div style={{ padding: "20px 24px 0", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 12 }}>
          ...
          <div><h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: "#f1f5f9" }}>{LABEL}</h2><p style={{ margin: 0, fontSize: 12, color: "#64748b" }}>Deep Character Psychology · {CHARACTERFORGE_ENGINES.length} engines · {CHARACTERFORGE_SERIES.length} series</p></div>
        </div>
