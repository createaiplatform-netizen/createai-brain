...
import {
  TIMELINE_ENGINES, TIMELINE_SERIES,
  runTimelineEngine,
  type TimelineEngineDefinition, type TimelineSeriesDefinition,
...
function EngineCard({ engine, onSelect }: { engine: TimelineEngineDefinition; onSelect: () => void }) {
  return (
...
function RunPanel({ engine, onBack }: { engine: TimelineEngineDefinition; onBack: () => void }) {
  ...
  const run = useCallback(async () => {
    ...
    await runTimelineEngine({ engineId: engine.id, engineName: engine.name, topic: topic.trim(),
  ...
  const save = useCallback(async () => {
    ...
    try { await fetch("/api/timelineforge", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ engineId: engine.id, engineName: engine.name, topic: topic.trim(), output }) }); setSaved(true); } catch { /* silent */ }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", gap: 16 }}>
      ...
      <button onClick={run} disabled={running || !topic.trim()} style={{ background: running ? "rgba(99,102,241,0.3)" : engine.color, border: "none", borderRadius: 10, padding: "11px 20px", color: "#fff", fontSize: 14, fontWeight: 700, cursor: running ? "not-allowed" : "pointer", opacity: !topic.trim() && !running ? 0.5 : 1 }}>
...
function SeriesPanel({ onBack }: { onBack: () => void }) {
  const [activeSeries, setActiveSeries] = useState<TimelineSeriesDefinition | null>(null);
  const [topic, setTopic] = useState(""); const [output, setOutput] = useState(""); const [running, setRunning] = useState(false);
  const outputRef = useRef<HTMLDivElement>(null);
  ...
  if (activeSeries) return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", gap: 16 }}>
      ...
      <textarea value={topic} onChange={e => setTopic(e.target.value)} placeholder="Enter your timeline topic for the full series run…" rows={3}
        style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 10, padding: "10px 14px", color: "#e2e8f0", fontSize: 13, fontFamily: "inherit", resize: "none" }} />
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: "#94a3b8", cursor: "pointer", fontSize: 20 }}>←</button>
        <div><div style={{ fontWeight: 700, fontSize: 16, color: "#f1f5f9" }}>⏰ Timeline Series</div><div style={{ fontSize: 12, color: "#64748b" }}>Run multiple engines in sequence on one historical arc</div></div>
      </div>
...
function SessionsPanel({ onBack, onView }: { onBack: () => void; onView: (s: SavedSession) => void }) {
  ...
  useEffect(() => { fetch("/api/timelineforge", { credentials: "include" }).then(r => r.ok ? r.json() : { sessions: [] }).then((d: { sessions?: SavedSession[] }) => setSessions(d.sessions ?? [])).catch(() => {}).finally(() => setLoading(false)); }, []);
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: "#94a3b8", cursor: "pointer", fontSize: 20 }}>←</button>
        <div style={{ fontWeight: 700, fontSize: 16, color: "#f1f5f9" }}>Saved Timelines ({sessions.length})</div>
      </div>
      {loading ? <div style={{ color: "#64748b", fontSize: 13 }}>Loading…</div>
        : sessions.length === 0 ? <div style={{ textAlign: "center", padding: "40px 0", color: "#64748b", fontSize: 13 }}>No saved timelines yet. Run an engine and save your history!</div>
        : sessions.map(s => (
...
export function TimelineForgeApp() {
  const [view, setView] = useState<View>("grid"); const [activeEngine, setActiveEngine] = useState<TimelineEngineDefinition | null>(null); const [viewingSession, setViewingSession] = useState<SavedSession | null>(null); const [search, setSearch] = useState("");
  const filtered = search.trim() ? TIMELINE_ENGINES.filter(e => e.name.toLowerCase().includes(search.toLowerCase()) || e.description.toLowerCase().includes(search.toLowerCase())) : TIMELINE_ENGINES;
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      <div style={{ padding: "20px 24px 0", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 12 }}>
          <div style={{ width: 44, height: 44, borderRadius: 12, background: "linear-gradient(135deg,#6366f1,#be185d)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22 }}>⏰</div>
          <div><h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: "#f1f5f9" }}>TimelineForge</h2><p style={{ margin: 0, fontSize: 12, color: "#64748b" }}>Temporal History Studio · {TIMELINE_ENGINES.length} engines · {TIMELINE_SERIES.length} series</p></div>
        </div>
        <div style={{ display: "flex", gap: 4 }}>
          {(["grid","series","sessions"] as const).map(v => (
            <button key={v} onClick={() => setView(v)} style={{ background: view === v ? "rgba(99,102,241,0.2)" : "transparent", border: view === v ? "1px solid rgba(99,102,241,0.4)" : "1px solid transparent", borderRadius: "8px 8px 0 0", padding: "7px 14px", color: view === v ? "#818cf8" : "#64748b", cursor: "pointer", fontSize: 12, fontWeight: view === v ? 700 : 400 }}>
