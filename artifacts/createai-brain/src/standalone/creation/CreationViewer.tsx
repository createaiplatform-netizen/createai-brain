...
function DownloadsSection({ creation }: { creation: Creation }) {
  ...
  const markDone = (id: string) => {
    ...
    setTimeout(() => setDone(prev => prev.filter(d => d !== id)), 2000);
  };

  const handleExportJSON = () => {
    const payload = {
      meta: { exportedAt: new Date().toISOString(), disclaimer: "All content is mock/simulated. Not for real use without expert review." },
      creation: { id: creation.id, name: creation.name, type: creation.type, domain: creation.domain,
        genre: creation.genre, style: creation.style, tone: creation.tone, createdAt: creation.createdAt,
        status: creation.status ?? "draft", tags: creation.tags ?? [], modules: creation.modules,
        sections: creation.sections.map(s => ({ title: s.title, content: s.content })) },
    };
    const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url;
    a.download = `${creation.name.replace(/[^a-z0-9]/gi, "_").toLowerCase()}_MOCK.json`;
  ...
  const handleExportMarkdown = () => {
    const lines = [
      ...
      `> ⚠️ **DISCLAIMER:** All content is mock/simulated. Generated by CreateAI Brain. Not for real use without expert review.`, ``,
      `---`, ``,
      ...creation.sections.flatMap(s => [`## ${s.title}`, ``, s.content, ``, `---`, ``]),
    ];
    const blob = new Blob([lines.join("\n")], { type: "text/markdown" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url;
    a.download = `${creation.name.replace(/[^a-z0-9]/gi, "_").toLowerCase()}_MOCK.md`;
    a.click(); URL.revokeObjectURL(url);
    markDone("markdown");
  };
  ...
  const handleCopyClipboard = async () => {
    const summary = [
      ...
      ``,
      ...creation.sections.map(s => `## ${s.title}\n${s.content.split("\n")[0].slice(0, 120)}…`),
      ``,
  ...
  const exportFormats = [
    { id: "json",      icon: "📦", label: "Export as JSON",     desc: "Structured data + metadata (.json)",       action: handleExportJSON },
    { id: "markdown",  icon: "📝", label: "Export as Markdown",  desc: "Human-readable .md with all sections",     action: handleExportMarkdown },
    { id: "clipboard", icon: "📋", label: "Copy Summary",        desc: "Markdown summary to clipboard",           action: handleCopyClipboard },
    { id: "full",      icon: "📄", label: "Download Full Text",  desc: "All sections as plain text (.txt)",        action: () => { downloadCreation(creation); markDone("full"); } },
  ];

  const sectionItems = creation.sections.slice(0, 5).map(s => ({
    id: `sec-${s.title}`, icon: "📑", label: s.title,
    desc: s.content.slice(0, 80) + "…",
    action: () => { downloadCreation(creation, s.title); markDone(`sec-${s.title}`); },
  }));

  return (
    <div className="p-5 md:p-6 space-y-5 max-w-3xl mx-auto">
      <div>
        <h2 className="text-xl font-bold text-foreground">Downloads & Exports</h2>
        <p className="text-[12px] text-muted-foreground mt-0.5">All exports are mock/demo only. No real data is transmitted.</p>
      </div>

      {/* Export Formats */}
      <div>
        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-2">Export Formats</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {exportFormats.map(item => (
            <button key={item.id} onClick={item.action}
              className={`flex items-center gap-3 p-4 rounded-2xl border text-left transition-all hover:shadow-sm ${done.includes(item.id) ? "bg-green-50 border-green-200" : "bg-background border-border/50 hover:border-primary/20"}`}>
              <div className="w-10 h-10 rounded-xl bg-muted flex items-center justify-center text-xl flex-shrink-0">{item.icon}</div>
              <div className="flex-1 min-w-0">
                <p className={`font-semibold text-[12px] ${done.includes(item.id) ? "text-green-700" : "text-foreground"}`}>
                  {done.includes(item.id) ? "✓ Done!" : item.label}
                </p>
      ...
      {sectionItems.length > 0 && (
        <div>
          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-2">Individual Sections (.txt)</p>
          <div className="space-y-1.5">
            {sectionItems.map(item => (
              <div key={item.id} className="flex items-center gap-3 p-3.5 bg-background rounded-xl border border-border/50">
                <span className="text-base">{item.icon}</span>
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-[12px] text-foreground truncate">{item.label}</p>
                  <p className="text-[10px] text-muted-foreground truncate">{item.desc}</p>
                </div>
                <button onClick={item.action}
                  className={`px-2.5 py-1 rounded-lg text-[10px] font-bold flex-shrink-0 transition-all ${done.includes(item.id) ? "bg-green-100 text-green-700" : "bg-primary/10 text-primary hover:bg-primary/20"}`}>
                  {done.includes(item.id) ? "✓" : "⬇ .txt"}
                </button>
              </div>
            ))}
          </div>
      ...
      <div className="bg-muted/40 rounded-2xl p-4 border border-border/30">
        <p className="text-[11px] text-muted-foreground">All exports are plain-text mock files for demonstration purposes only. PDF, Word, and production-ready formats require Live mode.</p>
      </div>
    </div>
  );
}
