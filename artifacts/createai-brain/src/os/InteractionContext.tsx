...
export function InteractionProvider({ children }: { children: React.ReactNode }) {
  ...
  const setIndustry   = useCallback((id: string)        => { InteractionEngine.setIndustry(id);    refresh(); }, [refresh]);
  const setCountry    = useCallback((c: string)         => { InteractionEngine.setCountry(c);      refresh(); }, [refresh]);
  const setDomain     = useCallback((d: string)         => { InteractionEngine.setDomain(d);       refresh(); }, [refresh]);
  const setMode       = useCallback((m: string)         => { InteractionEngine.setMode(m);         refresh(); }, [refresh]);
  const setScenario   = useCallback((s: string | null)  => { InteractionEngine.setScenario(s);     refresh(); }, [refresh]);
  ...
  return (
    <Ctx.Provider value={{
      ...
      setView, setUserType, setPacket, setDemoStatus,
      setIndustry, setCountry, setDomain, setMode, setScenario,
      dispatch, clearLog, reset, refresh,
    }}>
      {children}
    </Ctx.Provider>
  );
