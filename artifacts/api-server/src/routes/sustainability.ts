...
// GET /api/sustainability/records — list records for current user
router.get("/records", async (req, res) => {
  ...
  try {
    const records = await db
      .select()
      .from(documents)
      .where(and(eq(documents.userId, userId), eq(documents.docType, "Sustainability")))
...
// POST /api/sustainability/records — create a record
router.post("/records", async (req, res) => {
  ...
  try {
    ...
    await db.insert(activityLog).values({
      ...
      label: title.slice(0, 80), icon: "📄", appId: "sustainability",
      meta: { recordId: record.id, suite: "sustainability" },
    }).catch(() => {});
...
// GET /api/sustainability/records/:id
...
// DELETE /api/sustainability/records/:id
...
router.get("/stats", async (req, res) => {
  ...
  try {
    const records = await db.select().from(documents)
      .where(and(eq(documents.userId, userId), eq(documents.docType, "Sustainability")));
    res.json({ ok: true, suite: "sustainability", totalRecords: records.length });
  } catch {
    res.status(500).json({ error: "Database error" });
  }
});
