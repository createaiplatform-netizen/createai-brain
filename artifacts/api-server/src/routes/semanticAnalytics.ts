  ...
  try {
    ...
    const domains = [
      ...
      },
      {
        ...
        gap: 132,
        status: "active",
        immediateActions: ["Checkout metadata enrichment ✅ live", "Bundle detection in store"],
        blockers: ["Upsell/cross-sell in checkout requires Stripe Payment Links config"],
        notes: "Stripe checkout live for all products",
      },
      {
      ...
      },
      {
        ...
        current: hasCustomers ? 65 : 28,
        theoretical: 480,
        gap: hasCustomers ? 415 : 452,
        status: hasCustomers ? "active" : "empty",
        immediateActions: ["Customer capture ✅ live", "LTV tracking ✅ live", "Revenue by format ✅ live"],
        blockers: ["Behavioral tracking (page dwell time)", "Churn prediction requires historical data", "Personalized recommendations require ML"],
        notes: `${crmStats.totalCustomers} customers · $${(crmStats.totalRevenueCents / 100).toFixed(2)} revenue`,
      },
      {
      ...
      },
      {
        ...
        gap: 295,
        status: "planned",
        immediateActions: ["Recurring price detection in registry", "Subscription product badge in store"],
        blockers: ["Stripe subscription products require manual creation in dashboard", "Subscriber-only content requires auth layer", "Dunning management requires failed payment webhooks"],
        notes: "No subscription products in current Stripe catalog",
      },
      {
        ...
        gap: 222,
        status: "active",
        immediateActions: ["Affiliate link generation ✅ live", "Click tracking ✅ live", "UTM parameter injection ✅ live"],
        blockers: ["Commission calculation requires purchase attribution (affiliate code in checkout metadata)", "Affiliate payout requires Stripe Connect or manual transfer", "Affiliate dashboard requires auth"],
        notes: "Foundation layer live — ready for first affiliate",
      },
      {
      ...
      },
      {
        ...
        status: "planned",
        immediateActions: [],
        blockers: ["PDF generation: OpenAI text → html2pdf or puppeteer", "Audio: TTS API (OpenAI TTS or ElevenLabs)", "Video: synthesis requires external API (Runway/HeyGen)", "Storage: object storage needed for all generated files"],
        notes: "Largest gap — architecture designed, requires new pipeline",
      },
      {
      ...
      },
      {
        ...
        gap: 202,
        status: "partial",
        immediateActions: ["Stripe webhook signature verification (code ready, needs STRIPE_WEBHOOK_SECRET secret)", "Input sanitization ✅ in place"],
        blockers: ["STRIPE_WEBHOOK_SECRET needs to be configured in Replit Secrets", "Rate limiting on store endpoints", "GDPR data export endpoint"],
        notes: "Add STRIPE_WEBHOOK_SECRET to harden webhook endpoint",
      },
      {
