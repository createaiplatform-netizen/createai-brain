...
// GET /api/product-design/records — list records for current user
router.get("/records", async (req, res) => {
  ...
  try {
    const records = await db
      .select()
      .from(documents)
      .where(and(eq(documents.userId, userId), eq(documents.docType, "ProductDesign")))
...
// POST /api/product-design/records — create a record
router.post("/records", async (req, res) => {
  ...
  try {
    const [record] = await db.insert(documents).values({
      ...
      tags: tags ?? "product-design",
    }).returning();
    await db.insert(activityLog).values({
      userId, action: "document_create",
      label: title.slice(0, 80), icon: "📄", appId: "product-design",
      meta: { recordId: record.id, suite: "product-design" },
    }).catch(() => {});
...
// GET /api/product-design/records/:id
...
// DELETE /api/product-design/records/:id
...
router.get("/stats", async (req, res) => {
  ...
  try {
    const records = await db.select().from(documents)
      .where(and(eq(documents.userId, userId), eq(documents.docType, "ProductDesign")));
    res.json({ ok: true, suite: "product-design", totalRecords: records.length });
  } catch {
    res.status(500).json({ error: "Database error" });
  }
});
