...
// GET /api/research-lab/records — list records for current user
router.get("/records", async (req, res) => {
  ...
  try {
    const records = await db
      .select()
      .from(documents)
      .where(and(eq(documents.userId, userId), eq(documents.docType, "ResearchLab")))
...
// POST /api/research-lab/records — create a record
router.post("/records", async (req, res) => {
  ...
  try {
    const [record] = await db.insert(documents).values({
      ...
      body: body ?? "",
      docType: "ResearchLab",
      tags: tags ?? "research-lab",
    }).returning();
    await db.insert(activityLog).values({
      userId, action: "document_create",
      label: title.slice(0, 80), icon: "📄", appId: "research-lab",
      meta: { recordId: record.id, suite: "research-lab" },
    }).catch(() => {});
...
// GET /api/research-lab/records/:id
...
// DELETE /api/research-lab/records/:id
...
router.get("/stats", async (req, res) => {
  ...
  try {
    const records = await db.select().from(documents)
      .where(and(eq(documents.userId, userId), eq(documents.docType, "ResearchLab")));
    res.json({ ok: true, suite: "research-lab", totalRecords: records.length });
  } catch {
    res.status(500).json({ error: "Database error" });
  }
});
