...
// GET /api/education/records — list records for current user
router.get("/records", async (req, res) => {
  ...
  try {
    const records = await db
      .select()
      .from(documents)
      .where(and(eq(documents.userId, userId), eq(documents.docType, "Education")))
...
// POST /api/education/records — create a record
router.post("/records", async (req, res) => {
  ...
  try {
    ...
    await db.insert(activityLog).values({
      ...
      label: title.slice(0, 80), icon: "📄", appId: "education",
      meta: { recordId: record.id, suite: "education" },
    }).catch(() => {});
...
router.get("/stats", async (req, res) => {
  ...
  try {
    const records = await db.select().from(documents)
      .where(and(eq(documents.userId, userId), eq(documents.docType, "Education")));
    res.json({ ok: true, suite: "education", totalRecords: records.length });
  } catch {
    res.status(500).json({ error: "Database error" });
  }
});
