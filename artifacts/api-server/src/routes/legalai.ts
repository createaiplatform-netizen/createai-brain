...
// GET /api/legal-ai/records — list records for current user
router.get("/records", async (req, res) => {
  ...
  try {
    const records = await db
      .select()
      .from(documents)
      .where(and(eq(documents.userId, userId), eq(documents.docType, "LegalAI")))
...
// POST /api/legal-ai/records — create a record
router.post("/records", async (req, res) => {
  ...
  try {
    ...
    await db.insert(activityLog).values({
      ...
      label: title.slice(0, 80), icon: "📄", appId: "legal-ai",
      meta: { recordId: record.id, suite: "legal-ai" },
    }).catch(() => {});
...
// DELETE /api/legal-ai/records/:id
...
router.get("/stats", async (req, res) => {
  ...
  try {
    const records = await db.select().from(documents)
      .where(and(eq(documents.userId, userId), eq(documents.docType, "LegalAI")));
    res.json({ ok: true, suite: "legal-ai", totalRecords: records.length });
  } catch {
    res.status(500).json({ error: "Database error" });
  }
});
